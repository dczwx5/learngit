...
public class DisplayObjectFinder
{
    ...
    private static var intendSpace : int = -2;

    public static function dumpLogStr( container : DisplayObjectContainer ) : String
    {
        intendSpace += 2;
        var num : int = container.numChildren;
        var str : String = "";
        for ( var i : int = 0; i < num; i++ )
        {
            ...
            if ( child )
            {
                for ( var j : int = 0; j < intendSpace; ++j )
                {
                    str += " ";
                }
                str = str + child.name + ", " + Object( child ).constructor + ", " + String( child ) + ", " + getQualifiedClassName( child ) + "\n";
            }

            if ( child is DisplayObjectContainer )
            {
                str += dumpLogStr( child as DisplayObjectContainer );
            }
        }
        intendSpace -= 2;
    ...
    public static function dumpLog( container : DisplayObjectContainer, log : Function = null ) : void
    {
        if ( null == log )
            log = PluginBase.log;

        log( dumpLogStr( container ) );
    }
