...
import kof.game.currency.CCurrencyEvent;
    ...
    import kof.game.currency.qq.views.CQQHallView;
     ...
     * 2017/6/26
     */
    public class CQQHallViewHandler extends CTweenViewHandler implements ISystemBundle {
        private var _QQHallView : CQQHallView = null;
        private var _bViewInitialized : Boolean = false;

        public function CQQHallViewHandler() {
            super( false );
        }

        public function get bundleID() : * {
            return SYSTEM_ID( KOFSysTags.QQ_HALL );
        }
        ...
        override protected function onInitializeView() : Boolean {
            ...
            if ( !_bViewInitialized ) {
                ...
                _QQHallView = new CQQHallView( system );
                _QQHallView.closeHandler = new Handler( _onClose );
                _QQHallView.uiCanvas = uiCanvas;
            }
        ...
        private function _onSystemBundleStart( event : CSystemBundleEvent ) : void {
            ...
            if ( pSystemBundleCtx ) {
                ...
                var handler : Handler = new Handler( (system.stage.getSystem( CUISystem ).getHandler( CTipsViewHandler ) as CTipsViewHandler).showQQTips, [ KOFSysTags.QQ_HALL ] );
                pSystemBundleCtx.setUserData( this, "tip_handler", handler );
                var bool : Boolean = (system.getBean( CQQClientDataManager ) as CQQClientDataManager).hasGetReward( KOFSysTags.QQ_HALL );
        ...
        private function _updateData( e : Event ) : void {
            ...
            if ( pSystemBundleCtx ) {
                var bool : Boolean = (system.getBean( CQQClientDataManager ) as CQQClientDataManager).hasGetReward( KOFSysTags.QQ_HALL );
                pSystemBundleCtx.setUserData( this, "notification", bool );
