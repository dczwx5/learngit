 ...
 * <listing>
 ...
 &lt;/rtf&gt;</listing>
 */
public class CRichTextField extends Sprite
{
    private var _width:Number;
    private var _height:Number;
    private var _textRenderer:CTextRender;
    private var _spriteRenderer:CSpriteRenderer;
    ...
    public function setEnabled(e:Boolean):void
    {
        if(!e)
        {
            _textRenderer.type = TextFieldType.DYNAMIC;
        }else
        {
            _textRenderer.type = TextFieldType.INPUT;
        }
    }
    ...
    {
        new CTextFieldColor(_textRenderer,uint(_textRenderer.defaultTextFormat.color),value,uint(_textRenderer.defaultTextFormat.color));
    }

    /**
     * 构造函数。
     */
    public function CRichTextField()
    {
        //text renderer
        _textRenderer = new CTextRender();
        addChild(_textRenderer);

        //sprite renderer
        _spriteRenderer = new CSpriteRenderer(this);
    ...
    public function append(newText:String, newSprites:Array = null, autoWordWrap:Boolean = false, format:TextFormat = null):void
    {
        ...
        if (newText || autoWordWrap)
        {
            ...
            if (html)
            {
//                trace(_textRenderer.htmlText ,'-------------------')
                _textRenderer.htmlText += newText ;
...
//                trace(_textRenderer.htmlText ,'+++++++++++++++')
    ...
    private function insertSprites(newSprites:Array, startIndex:int, maxIndex:int, newline:Boolean = false):void
    {
        ...
        var sortB : Boolean;
        var objI : int = -1;
        for each( var o : Object in newSprites ){
            if( objI == -1 ){
                objI = o.i;
            }
            if( o.i != objI ){
        ...
        //
        if( sortB )
            newSprites.sortOn("i", Array.NUMERIC);

        for (var i:int = 0; i < newSprites.length; i++)
        {
            var obj:Object = newSprites[i];
            var sprite:Object = obj.r;
            var index:int = obj.i;
            if (obj.i == undefined || index < 0 || index > maxIndex - startIndex)
            {
                obj.index = obj.i = maxIndex - startIndex;
            ...
            if (newline && index > 0 && index < maxIndex - startIndex) index += startIndex + i;
    ...
    public function set faceRect(value:Rectangle):void
    {
        _rect = value;
    }
    ...
    public function insertSprite(newSprite:Object, index:int = -1, autoRender:Boolean = true, cache:Boolean = false):void
    {
        ...
        if(_rect)
        {
            format = calcPlaceholderFormat(_rect.width, _rect.height);
        }else
        {
            format = calcPlaceholderFormat(spriteObj.width, spriteObj.height);//
        }
        ...
        _textRenderer.width
        _textRenderer.textWidth;
        _spriteRenderer.insertSprite(spriteObj, index);
        if(this.stage)
        {
            this.stage.focus = _textRenderer;
        }
    ...
    public function getResetY():Number
    {
        var metrics:TextLineMetrics = _formatCalculator.getLineMetrics(0);
    ...
    public function clear():void
    {
        ...
        _textRenderer.clear();
        if(_textRenderer.type == TextFieldType.INPUT)
        {
            _textRenderer.recoverDefaultFormat();
        }
    }
    ...
    public function set type(value:String):void
    {
        ...
        _textRenderer.addEventListener(Event.SCROLL, scrollHandler, false, 0, true);
        if (value == INPUT)
        {
            _textRenderer.addEventListener(Event.CHANGE, changeHandler, false, 0, true);
        }
    }
     ...
     * @return
     */
    public function exportXML( offsetI : int = 0 ):XML
    {
        var xml:XML = <a/>;
        if (html)
        {
            xml.ht = _textRenderer.htmlText.split(_placeholder).join("");
        }else
        {
            xml.t = _textRenderer.text.split(_placeholder).join("");
        }
        xml.s = _spriteRenderer.exportXML( offsetI );
        return xml;
    }
     ...
     */
    public function importXML(data:XML,replacePlace:Boolean = false,offSetI : int = 0):void
    {
        var content:String = "";
        if ("ht" in data)
        {
        ...
        var sprites:Array = [];
        for (var i:int = 0; i < data.s.f.length(); i++)
        {
            var node:XML = data.s.f[i];
            var sprite:Object = { };
            sprite.r = String(node.@r);
            //correct the index if import as html
            //if (html) sprite.index = int(node.@index) + 1;
            //else sprite.index = int(node.@index);

            sprite.i = int(node.@i) + offSetI;
            sprites.push(sprite);
            if(replacePlace)
            {
                content = content.slice(0,sprite.i)+content.slice(sprite.i+1);
            }
        }
