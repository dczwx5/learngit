...
public class CTotalRechargeSystem extends CBundleSystem {

    private var m_bInitialized : Boolean;

    private var m_pViewHandler:CTotalRechargeViewHandler;
    private var m_pHelpHandler:CTotalRechargeHelperHandler;
//    private var m_pNetHandler:CDailyRechargeNetHandler;
//    private var m_pManager:CDailyRechargeManager;

    private var m_pValidater : CTotalRechargeValidater;

    public function CTotalRechargeSystem( A_objBundleID : * = null ) {
        super( A_objBundleID );
    }
    ...
    override public function initialize() : Boolean
    {
        ...
        if ( !m_bInitialized )
        {
            ...
            m_pViewHandler = new CTotalRechargeViewHandler();
            this.addBean( m_pViewHandler );

            m_pHelpHandler = new CTotalRechargeHelperHandler();
...
//            this.addBean( m_pManager );
            ...
            m_pValidater = new CTotalRechargeValidater(this);
            switchingSystem.addValidator( m_pValidater );
        }

        m_pViewHandler = m_pViewHandler || this.getHandler( CTotalRechargeViewHandler ) as CTotalRechargeViewHandler;
...
//        addEventListener( CActivityHallEvent.ConsumeActivityResponse, _updateRedPoint );
    ...
    override protected function onActivated( value : Boolean ) : void
    {
        ...
        var pView : CTotalRechargeViewHandler = this.getHandler( CTotalRechargeViewHandler ) as CTotalRechargeViewHandler;
        if ( !pView )
        {
    ...
    {
        return SYSTEM_ID( KOFSysTags.TOTAL_RECHARGE );
    }
    ...
    private function _updateActivityOpenState(event : CActivityHallEvent = null):void
    {
        ...
        {
            if((stage.getSystem(CSwitchingSystem) as CSwitchingSystem).isSystemOpen(KOFSysTags.TOTAL_RECHARGE))
            {
                return;
            }
            ...
            if ( pValidators )
            {
                if ( pValidators.evaluate() )// 验证所有开启条件是否已达成
                {
                    ...
                    {
                        if(vResult.indexOf(KOFSysTags.TOTAL_RECHARGE) != -1)
                        {
                            var pSystemBundleContext : ISystemBundleContext = stage.getBean( ISystemBundleContext ) as ISystemBundleContext;
