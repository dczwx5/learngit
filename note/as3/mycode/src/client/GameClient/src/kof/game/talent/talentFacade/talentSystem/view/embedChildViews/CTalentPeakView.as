 ...
 * Created by yili(guoyiligo@qq.com) on 2017/10/18.
 * Time: 12:21
 */
...
public class CTalentPeakView {
    ...
    private var pRecordPropertyAdd : Dictionary = new Dictionary();
    private var _talentTotalLv : int = 0;
    private var _isShowTabRedPt:Boolean = false;
    private var _propertyData:CBasePropertyData;

    public function CTalentPeakView( talentMainView : CTalentMainView) {
    ...
    public function update() : void {
        ...
        {
//            _talentMainView.talentUI.tabRed2.visible = false;
            return;
        }

        var talentPageData : CTalentAllPointData = CTalentDataManager.getInstance().getTalentPagePointData( ETalentPageType.PEAK );
        var i : int = 0;
        var openLv : int = 0;
        var talenPointTypeDesc : String = "";
        var data : Object = null;
        var talentIcoUI : TalentIco2UI = null;
        var talentPointSoul : TalentSoulPoint = null;
        _isShowTabRedPt = false;
        if ( talentPageData ) {
            ...
            for ( var j : int = 0; j < len; j++ ) {
                ...
                for ( i = 1; i <= 16; i++ ) {
                    if ( i == talentPointSoul.pointID ) {
                        talentIcoUI = _talentUI.peak.getChildByName( "item" + i ) as TalentIco2UI;
                        talentPointSoul = CTalentFacade.getInstance().getTalentPointSoulForPointIDAndPage( i ,ETalentPageType.PEAK);
                        ...
                        if ( talentPointData.state == ETalentPointStateType.OPEN_CAN_EMBED ) {
                            ...
                            if ( CTalentDataManager.getInstance().getTalentPointForWarehouse( i , ETalentPageType.PEAK).length ) {
                                ...
                                _isShowTabRedPt = true;
                            } else {
                                talentIcoUI.img_red.visible = false;
                            }
                            talentIcoUI.kuangClip.index = CTalentFacade.getInstance().getPeakTalentGridBgColorForTalentMainType( talentPointSoul.mainType )+3;
                        }
                            ...
                            talentIcoUI.kuangClip.index = CTalentFacade.getInstance().getPeakTalentGridBgColorForTalentMainType( talentPointSoul.mainType )+6;
                        }
            ...
            for ( i = 1; i <= 16; i++ ) {
                if ( _alreadyOpenPointID.indexOf( i ) == -1 ) {
                    ...
                    talentPointSoul = CTalentFacade.getInstance().getTalentPointSoulForPointIDAndPage( i ,ETalentPageType.PEAK);
                    var colorIndexNotOpen : int = CTalentFacade.getInstance().getPeakTalentGridBgColorForTalentMainType( talentPointSoul.mainType );
                    talentIcoUI.kuangClip.index = colorIndexNotOpen;
                }
            }
            //下一个开启的位置
            var nextPointArr:Array=CTalentFacade.getInstance().nextOpenSePointID(ETalentPageType.PEAK);
            var nextPoint : int = nextPointArr.length>0?nextPointArr[0]:0;
            if ( nextPoint > 0 ) {
                talentIcoUI = _talentUI.peak.getChildByName( "item" + nextPoint ) as TalentIco2UI;
                talentPointSoul = CTalentFacade.getInstance().getTalentPointSoulForPointIDAndPage( nextPoint ,ETalentPageType.PEAK);
                openLv = this._talentMainView.getTalentPointOpneLv( talentPointSoul.pointID ,ETalentPageType.PEAK);
                var isCanOpen:Boolean = _helper.isTalentCanOpen(talentPointSoul);
//                if ( openLv <= CTalentFacade.getInstance().teamLevel ) {
                ...
                talentIcoUI.lvClipHuang.visible = false;
                talentIcoUI.lvClipHong.visible = false;
                talentIcoUI.kuangClip.index = CTalentFacade.getInstance().getPeakTalentGridBgColorForTalentMainType( talentPointSoul.mainType );
                talentIcoUI.ico.visible = false;
                talentIcoUI.btn.visible = true;
                ...
                talentIcoUI.btn.toolTip = new Handler( _talentMainView.showTips, [ data ] );
                var colorIndexNextOpen : int = CTalentFacade.getInstance().getPeakTalentGridBgColorForTalentMainType( talentPointSoul.mainType );
            ...
            for ( i = 1; i <= 16; i++ ) {
                ...
                if ( i == 1 ) {
                    ...
                    talentIcoUI.ico.visible = false;
                    talentIcoUI.btn.visible = true;
                    talentIcoUI.btn.clickHandler = new Handler( _talentMainView.openTalentPoint, [ i , talentPointSoul.ID] );
                    openLv = _talentMainView.getTalentPointOpneLv( talentPointSoul.pointID ,ETalentPageType.PEAK);
                    talentIcoUI.txt1.text = "Lv." + openLv;
                    talenPointTypeDesc = _talentMainView.getTalentPointTypeDesc( i );
                    data = {};
                    data.talenPointTypeDesc = talenPointTypeDesc;
...
//                    if(openLv <= CTalentFacade.getInstance().teamLevel){
        ...
        if(_isShowTabRedPt || isShowTabRedPoint2){
//            _talentMainView.talentUI.tabRed2.visible = true;
        }else{
//            _talentMainView.talentUI.tabRed2.visible = false;
        }
        if(_talentMainView.currentPage==ETalentPageType.PEAK)
        {
            _updateProertyValue();
        ...
        if(_talentUI.clip_suit)
        {
            ...
            _talentUI.clip_suit.toolTip = new Handler(_talentMainView.showSuitTips, [ETalentPageType.PEAK]);
        }
        ...
        _talentUI.txt_totalLevelLabel.text = "拳皇大赛斗魂总等级";
    }
    ...
    private function _updateSpecialTalent():Boolean
    {
        ...
        for(var i:int = 17; i <= 19; i++)
        {
            var talentIcoUI:TalentIco3UI = _talentUI.getChildByName( "specialItem_" + (i-16) ) as TalentIco3UI;
            if(_alreadyOpenPointID.indexOf(i) != -1)// 已开启
            {
                var talentSoulPoint:TalentSoulPoint = CTalentFacade.getInstance().getTalentPointSoulForPointIDAndPage( i ,ETalentPageType.PEAK);
                talentIcoUI.txt1.visible = false;
                talentIcoUI.txt2.visible = false;
                talentIcoUI.btn.visible = true;
...
//                    talentIcoUI.kuangClip.index = CTalentFacade.getInstance().getBenYuanTalentGridBgColorForTalentMainType( talentSoulPoint.mainType )+6;
            ...
            {
                talentIcoUI = _talentUI.getChildByName("specialItem_" + (i-16)) as TalentIco3UI;
                talentIcoUI.txt1.visible = false;
                talentIcoUI.txt2.visible = false;
                talentIcoUI.ico.visible = false;
                talentIcoUI.btn.visible = false;
                talentIcoUI.btn.clickHandler = null;
                talentSoulPoint = CTalentFacade.getInstance().getTalentPointSoulForPointIDAndPage( i , ETalentPageType.PEAK);
    ...
    private function _updateProertyValue() : void {
        ...
        var allTalentPointInfos : CTalentAllPointData = CTalentDataManager.getInstance().getTalentPagePointData( ETalentPageType.PEAK );
        if ( allTalentPointInfos ) {
            ...
            var suitInfo:TalentSoulSuit = _helper.getCurrSuitInfo(ETalentPageType.PEAK);
...
//        }
        ...
        _talentUI.num_suitLevel.num = _helper.getCurrSuitLevel(ETalentPageType.PEAK);
    }
...
//        ((_talentUI.getChildByName( "cb" ) as Box).getChildByName( "cb3" ) as Box).visible = false;
    ...
    private function _valueRender( item : Component, idx : int ) : void {
        ...
        var passiveSkillPro : PassiveSkillPro = CTalentFacade.getInstance().getPassiveSkillProData( propertyID );
        (box.getChildByName( "txt" ) as Label).text = "+" + int( _talentMainView.pRecordPropertyAdd[ propertyID ].addValue );
    }
