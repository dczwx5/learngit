...
/**全局引用入口*/
	public class App {
		...
		/**获得资源路径(此处可以加上资源版本控制)*/
		public static function getResPath(url:String, retryNum : int = 0 ):String {
			var ret : String = /^http:\/\//g.test(url) ? url : Config.resPath + url;
			if ( ret && CResourceLoaders.instance().assetVersion ) {
				ret = CResourceLoaders.instance().assetVersion.mappingFilenameWithVersion( ret.toLowerCase() );
                if ( retryNum > 0 && ret.indexOf('?') != -1 ) {
                    ret += "_" + retryNum;
				}
			}
			return ret;
