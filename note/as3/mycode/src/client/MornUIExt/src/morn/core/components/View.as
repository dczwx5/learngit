...
public class View extends Box {
    ...
    public static function createComp( xml : XML, comp : Component = null, view : View = null ) : Component {
        ...
        for each ( var attrs : XML in xml.attributes() ) {
            ...
            // XXX: KOF HACK START
                ...
                // push the non-declarations property into "tags", use in custom
                // parsing.
                comp.tag = comp.tag || {};
                comp.tag[ prop ] = value;
            }
        ...
        // XXX: KOF HACK START
        if ( comp.tag && compCustomParsers.length ) { // contains custom tag defines.
            for each (var parser : Function in compCustomParsers ) {
                if ( null != parser )
                    parser( comp );
            }
        }
    ...
    // XXX: KOF HACK START
    private static var compCustomParsers : Vector.<Function> = new <Function>[];

    /**注册自定义附加属性解析器*/
    public static function registerComponentCustomParser( parser : Function ) : void {
        compCustomParsers.push( parser );
    }
