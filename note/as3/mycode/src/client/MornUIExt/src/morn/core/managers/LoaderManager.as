...
public class LoaderManager extends EventDispatcher {
    ...
    private function checkNext():void {
        ...
        while (_resInfos.length > 0) {
            ...
            if (content != null) {
                ...
                _resLoader.load(resInfo.url, resInfo.type, new Handler(loadComplete, [resInfo]), resInfo.progress, resInfo.isCacheContent, resInfo.retryNum);
                return;
            }
        }
        _isLoading = false;
    ...
    public function loadAssets(arr:Array, complete:Handler = null, progress:Handler = null, error:Handler = null, isCacheContent:Boolean = true):void {
        ...
        for (var i:int = 0; i < itemCount; i++) {
            ...
            totalSize += item.size;
            load(item.url.toString(), item.type, new Handler(loadAssetsComplete, [item.size]), new Handler(loadAssetsProgress, [item.size]), error, isCacheContent);
        }
