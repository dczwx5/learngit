...
/**资源加载器*/
	public class ResLoader {
        ...
        public static var checkOpenTimeOut:int = 15000;
		public static var checkProgressTimeOut:int = 5000;
        ...
        private var _imgLoaderContext:LoaderContext = new LoaderContext(false, ApplicationDomain.currentDomain);
		...
		public function ResLoader() {
			...
			_imgLoaderContext.allowCodeImport = false;
			_imgLoaderContext.imageDecodingPolicy = ImageDecodingPolicy.ON_DEMAND;
		}
		...
		private function doLoad():void {
			_isLoading = true;
			_urlRequest.url = App.getResPath(_url, _retryNum);
			if (_type == AMF || _type == DB || _type == BYTE || _type == SWF) {
			...
			if( _type == BMD ) {
				...
				try {
					_loader.load( _urlRequest, _imgLoaderContext );
		...
		private function onStatus(e:HTTPStatusEvent):void {
			// App.log.debug( "[HTTPStatus] (" + _url + "): " + e.status );
		}
		...
		private function onOpen(e:Event):void {
			App.timer.clearTimer( checkLoad );
			App.timer.doLoop( checkProgressTimeOut, checkLoad );
		}
		...
		private function onComplete(e:Event):void {
			...
			if (_type == SWF /* || _type == BMD */) {
				...
				if ( _type == SWF ) {
					_loader.unloadAndStop( Boolean( Sys.stage ) );
					content = 1;
				} else {
		...
		/**加载资源*/
		public function load(url:String, type:int, complete:Handler, progress:Handler, isCache:Boolean = true, retryNum:int = 0):void {
			if (_isLoading) {
			...
			App.timer.doLoop(checkOpenTimeOut, checkLoad);
			doLoad();
		}
