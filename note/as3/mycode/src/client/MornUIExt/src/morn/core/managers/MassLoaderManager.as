...
package morn.core.managers {
	...
	public class MassLoaderManager extends EventDispatcher {
		...
		private function doLoad(resInfo:ResInfo):void {
			...
			resLoader.load(resInfo.url, resInfo.type, new Handler(loadComplete, [resLoader, resInfo]), new Handler(loadProgress, [resInfo]), resInfo.isCacheContent, resInfo.retryNum);
		}
		...
		private function endLoad(resInfo:ResInfo, content:*):void {
			...
			if (content == null) {
				...
				if (errorCount < _retryNum) {
					resInfo.retryNum = _failRes[resInfo.url] = errorCount + 1;
		...
		public function loadAssets(arr:Array, complete:Handler = null, progress:Handler = null, error:Handler = null, isCacheContent:Boolean = true):void {
			...
			for (var i:int = 0; i < itemCount; i++) {
				...
				totalSize += item.size;
				items.push(item);
				load(String(item.url), item.type, item.priority, new Handler(loadAssetsComplete, [item]), new Handler(loadAssetsProgress, [item]), error, isCacheContent);
			}
