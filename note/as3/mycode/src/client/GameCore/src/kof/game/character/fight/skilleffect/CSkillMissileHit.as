 ...
 */
public class CSkillMissileHit extends CAbstractSkillEffect {
    public function CSkillMissileHit( id : int, startFrame : Number, hitEvent : String, etype : int, des : String = "" ) {
    ...
    private function _absorbMissile() : void {
        ...
        for each ( var colTarget : CGameObject in m_collisionTargets ) {
            ...
            if ( ( absorbInfo != null && !absorbInfo.bTimeToAbsort) )
                continue;
            rets.push( colTarget );
        }
    ...
    private function _doAbsorbTargets( targets : Array ) : void {
        ...
        for each( var target : CGameObject in targets ) {
            ...
            _shakeCamero( m_aeroAbsorbInfo.HitCameraEffect);
    ...
    private function _shakeCamero( shakeID : int ) : void {
        if( shakeID == 0 )
                return;
    ...
    private function _findMissile() : Boolean {
    ...
    private var m_collisionTargets : Array;
...
class AbsorbingTargetInfo implements IUpdatable {
    ...
    public function get bTimeToAbsort() : Boolean {
