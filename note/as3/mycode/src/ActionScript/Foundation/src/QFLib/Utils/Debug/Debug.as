	...
	public final class Debug
	{
		...
		public static function monitor(stage:Stage, pollInterval:int = 500):void
		{
			...
			if (_isEnabled && !_fpsMeter)
			{
				...
				_fpsMeter.addEventListener(FPSMeter.FPS_UPDATE, onFPSUpdate, false, 0, true);
				_fpsMeter.start();
			}
		}
		...
		private static function send(m:String, d:*, l:int = 1, r:int = 0):void
		{
			...
			if (_isEnabled)
			{
				...
				if (!_isConnected)
				{
					...
					_connection.addEventListener(StatusEvent.STATUS, onStatus, false, 0, true);
				}
