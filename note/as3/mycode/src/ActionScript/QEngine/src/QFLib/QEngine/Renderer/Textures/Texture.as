...
package QFLib.QEngine.Renderer.Textures
{
    ...
    import QFLib.QEngine.Renderer.Utils.GetNextPowerOfTwo;
     ...
     *
     *  @see QFLib.Graphics.RenderCore.starling.display.Image
     *  @see QFLib.Graphics.RenderCore.starling.utils.AssetManager
     *  @see QFLib.QEngine.Renderer.Textures.TextureAtlas
     */
    public class Texture
    {
        private static var _bufBmd : BitmapData;
        ...
        public static function fromEmbeddedAsset( assetClass : Class, mipMapping : Boolean = true,
                                                  ...
                                                  scale : Number = 1, format : String = "compressedAlpha",
        ...
        {
            if( !RenderDeviceManager.getInstance().current.contextValid ) throw new MissingContextError();

            var atfData : AtfData = new AtfData( data );

            if( atfData.width < 4 || atfData.height < 4 )
            {
                if( !_bufBmd ) _bufBmd = new BitmapData( 4, 4, true, 0x1 );

                var texture : ConcreteTexture = empty( 4, 4, false ) as ConcreteTexture;

                texture.uploadBitmapData( _bufBmd );
            ...
            if( atfData.format == Context3DTextureFormat.BGRA )
            {
                premultiplyAlpha = true;
            }

            var concreteTexture : ConcreteTexture = new ConcreteTexture( atfData.format,
                    atfData.width, atfData.height, useMipMaps && atfData.numTextures > 1,
                    premultiplyAlpha, false, scale, repeat );
        ...
        public static function fromColor( width : Number, height : Number, color : uint = 0xffffffff,
                                          optimizeForRenderToTexture : Boolean = false,
                                          scale : Number = -1, format : String = "compressedAlpha" ) : Texture
        {
        ...
        {
            var current : RenderDevice = RenderDeviceManager.getInstance().current;
            if( scale <= 0 ) scale = current.contentScaleFactor;

            var actualWidth : int, actualHeight : int;

            if( !current.contextValid ) throw new MissingContextError();
            ...
            //保证宽高不会超过2048
            var maxLength : Number = actualWidth > actualHeight ? actualWidth : actualHeight;
            if( maxLength > 2048.0 )
            {
                var factor : Number = 2048.0 / maxLength;
                actualWidth = int( actualWidth * factor );
                actualHeight = int( actualHeight * factor );
            ...
            if( optimizeForRenderToTexture && format == "compressedAlpha" )
                format = "bgra";

            var concreteTexture : ConcreteTexture = new ConcreteTexture( format,
        ...
        public function Texture()
        {
            if( Capabilities.isDebugger &&
                    getQualifiedClassName( this ) == "QFLib.Graphics.RenderCore.starling.textures::Texture" )
            {
                throw new AbstractClassError();
            }

            m_DeviceManager = RenderDeviceManager.getInstance();
        }

        protected var m_DeviceManager : RenderDeviceManager = null;
        private var m_Disposed : Boolean = false;
        private var m_Repeat : Boolean = false;
        ...
        /** Some texture (e.g. Core.spine) should ignore frame information when call adjustVertexData.*/
        public function set ignoreFrame( value : Boolean ) : void
        {
        }
