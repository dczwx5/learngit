...
{
    import QFLib.QEngine.Renderer.Utils.MatrixUtil;
    import QFLib.QEngine.Renderer.Utils.RectangleUtil;
    ...
    public class SubTexture extends Texture
    {
        ...
        private var mIgnoreFrame : Boolean = false;
        /** The clipping rectangle, which is the region provided on initialization
         *  scaled into [0.0, 1.0]. */
        private var mClipBuf : Rectangle;

        public override function set ignoreFrame( value : Boolean ) : void
        {
            mIgnoreFrame = value;
        }
        ...
        public override function get disposed() : Boolean
        {
            return mParent.disposed;
        }
        ...
        {
            if( mClipBuf == null )//buffered for optimization
            {
                ...
                mClipBuf = new Rectangle( topLeft.x, topLeft.y,
        ...
        public override function adjustVertexData( vertexData : VertexData, vertexID : int, count : int ) : void
        {
            ...
            var stride : int = VertexData.ELEMENTS_PER_VERTEX - 3;

            adjustTexCoords( vertexData.rawData, startIndex, stride, count );

            if( !mIgnoreFrame && mFrame )
            {
                if( count != 4 )
                ...
                vertexData.translateVertex( vertexID, -mFrame.x, -mFrame.y, 0 );
                vertexData.translateVertex( vertexID + 1, -deltaRight, -mFrame.y, 0 );
                vertexData.translateVertex( vertexID + 2, -mFrame.x, -deltaBottom, 0 );
                vertexData.translateVertex( vertexID + 3, -deltaRight, -deltaBottom, 0 );
            }
        }
