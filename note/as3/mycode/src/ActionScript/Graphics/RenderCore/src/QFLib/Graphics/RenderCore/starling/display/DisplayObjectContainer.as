...
use namespace starling_internal;
    ...
    public class DisplayObjectContainer extends DisplayObject
    {
		...
		private var mChildAddListener:Function = null;
        
        /** Helper objects. */
		//这个是给hittest用的，
        protected static var sHitTestPoint:Point = new Point();
        ...
        {
            // base object uses DisplayObjectContainer as a container
            /*if (Capabilities.isDebugger &&
                getQualifiedClassName(this) == "QFLib.Graphics.RenderCore.starling.display::DisplayObjectContainer")
            {
        ...
        public override function dispose():void
        {
            if (Version.isDebugBuild())
            {
                ...
                {
                    throw new Error("mChildren.length must be zero! Otherwise something wrong!");
                }
            }
            else
            {
                for (i = mChildren.length - 1; i >= 0; --i)
                {
                    mChildren[i].dispose()
                }
		...
		public function setChildAddListener(listener:Function):void
		{
			mChildAddListener = listener;
		}
		
		public function getChildAddListener():Function
		{
			return mChildAddListener;
		}
        ...
        public function replaceChildAt( child:DisplayObject, idx : int ):DisplayObject
        {
            if (child.inheritRenderQueue)
                child.renderQueueID = this.renderQueueID;

            if (usingCamera != null)
                child.usingCamera = usingCamera;

            if (layer != 0)
                child.layer = layer;

            if (mChildAddListener != null)
                mChildAddListener(child);
...
//            }
		...
		/*
        public function updateChildrenAt(index:int, child:DisplayObject):void
        {
        ...
        */
        ...
        public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle=null):Rectangle
        {
            ...
            {
                resultRect.copyFrom(mChildren[0].getBounds(targetSpace, resultRect));
            }
            else
            {
        ...
        public override function hitTest(localPoint:Point, forTouch:Boolean=false):DisplayObject
        {
            ...
            for (var i:int=numChildren-1; i>=0; --i) // front to back!
            {
                ...
                MatrixUtil.transformCoords(sHelperMatrix, localX, localY, sHitTestPoint);
                var target:DisplayObject = child.hitTest(sHitTestPoint, forTouch);
        ...
        public override function render(support:RenderSupport, parentAlpha:Number):void
        {
            ...
            for (var i:int=0; i<numChildren; ++i)
            {
                ...
                if (child.hasVisibleArea)
                {
                    if ( filterEnable )
                    {
                        var filter:ObjectFilter = child.objectFilter;
                        if (filter != null && filter.enable)
                        {
                            filter.render(support, alpha);
                        }
                    }
                    else
		...
		public override function isDrawSelf():Boolean 
		{
			return false; 
		}
		
		public override function get childCount():int
		{
			return mChildren.length;
		}
		
		public override function getChild(index:int):ISceneNode
		{
//			if( mChildren[index].filter)
//			{
//				return mChildren[index];
//				// 需要改造fitler，至于是把filter改造成ISceneNode
//				// 还是改成material，还没定论，看引擎的发展。我更倾向于后者。
//				//return  mChildren[index].filter;
//			}
//			else
//			{
				return mChildren[index];
//			}
		...
		override public function updateUnify(dt:Number):void
		{
			super.updateUnify(dt);
			for(var i:int  = 0, n:int = childCount; i < n;++i)
			{
                mChildren[i].updateUnify(dt);
			}
		}
		
		override public function set finalAlphaDirty(value:Boolean):void
		{
			if (super.finalAlphaDirty != value)
			{
				super.finalAlphaDirty = value;
				
				if(value)
				{
					for(var i:int  = 0, n:int = childCount; i < n;++i)
					{
						mChildren[i].finalAlphaDirty = true;
					}
				}
			}
		}
		
		override public function set worldMatrixDirty(value:Boolean):void
		{
			if (super.worldMatrixDirty != value)
			{
				super.worldMatrixDirty = value;
				
				if(value)
				{
					for(var i:int  = 0, n:int = childCount; i < n;++i)
					{
						mChildren[i].worldMatrixDirty = true;
					}
				}
			}
		}
		
		override public function set usingCamera(c:ICamera):void
		{
			super.usingCamera = c;
			for(var i:int = 0, n:int = numChildren; i < n; i++)
			{
				mChildren[i].usingCamera = c;
			}
		}
