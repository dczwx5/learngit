...
package QFLib.Graphics.RenderCore.starling.display
{
    ...
    import QFLib.Graphics.RenderCore.starling.events.TouchEvent;
    ...
    import QFLib.Graphics.RenderCore.starling.utils.MemoryTrace;
    ...
    [Event(name="added", type="QFLib.Graphics.RenderCore.starling.events.Event")]
    
    /** Dispatched when an object is connected to the stage (directly or indirectly). */
    [Event(name="addedToStage", type="QFLib.Graphics.RenderCore.starling.events.Event")]
    
    /** Dispatched when an object is removed from its parent. */
    [Event(name="removed", type="QFLib.Graphics.RenderCore.starling.events.Event")]
    
    /** Dispatched when an object is removed from the stage and won't be rendered any longer. */ 
    [Event(name="removedFromStage", type="QFLib.Graphics.RenderCore.starling.events.Event")]
    
    /** Dispatched once every frame on every object that is connected to the stage. */ 
    [Event(name="enterFrame", type="QFLib.Graphics.RenderCore.starling.events.EnterFrameEvent")]
    
    /** Dispatched when an object is touched. Bubbles. */
    [Event(name="touch", type="QFLib.Graphics.RenderCore.starling.events.TouchEvent")]
    
    /** Dispatched when a key on the keyboard is released. */
    [Event(name="keyUp", type="QFLib.Graphics.RenderCore.starling.events.KeyboardEvent")]
    
    /** Dispatched when a key on the keyboard is pressed. */
    [Event(name="keyDown", type="QFLib.Graphics.RenderCore.starling.events.KeyboardEvent")]
     ...
     *  @see Stage 
     */
    public class DisplayObject extends EventDispatcher implements ISceneNode
    {
        ...
        private var mFilter : ObjectFilter = null;
        private var mFilterEnable : Boolean = false;
		
		private var mLocalMatrix:Matrix;
		private var mWorldMatrix:Matrix;
		private var mLocalMatrixDirty:Boolean;
		private var mWorldMatrixDirty:Boolean;
		private var mFinalAlpha:Number = 1.0;
		private var mFinalAlphaDirty:Boolean = true;
		private var mUsingCamera:ICamera = null;
		private var mIsInRender:Boolean = true;

        private var mLayer:uint = 0;
		private var mRenderQueueID:int = RenderQueueGroup.DEFAULT;
		private var mInheritRenderQueue:Boolean = true;

        protected var mUniqueName:String;
        protected var mPreRenderFunc:Function = null;
        ...
        public function DisplayObject()
        {
            if (Capabilities.isDebugger && 
                getQualifiedClassName(this) == "QFLib.Graphics.RenderCore.starling.display::DisplayObject")
            {
			...
			if (EnableStackTrace)
			{
				...
				Stastics[mUniqueName] = new MemoryTrace(new Error().getStackTrace(), this);
			}
        }
        ...
        public function dispose():void
        {
            ...
            {
				return;
                throw new Error("DisplayObject dispose twice!");
            }
            ...
            if (Version.isDebugBuild())
            {
                if (mParent != null)
                {
                    if (mParent.getChildIndex(this) != -1)
                    {
                        throw new IllegalOperationError("You should dispose DisplayObject by removeFromParent(true)!");
                    }
                }
            }
			mParent = null;
                                                ...
                                                resultMatrix:Matrix=null):Matrix
        {
            ...
            {
                resultMatrix.copyFrom(localTransform);
                return resultMatrix;
            }
            else if (targetSpace == null || targetSpace == base)
            {
                ...
                {
                    resultMatrix.concat(currentObject.localTransform);
            ...
            while (currentObject != commonParent)
            {
                sHelperMatrix.concat(currentObject.localTransform);
                currentObject = currentObject.mParent;
            }
        ...
        public function hitTest(localPoint:Point, forTouch:Boolean=false):DisplayObject
        {
			...
			var bound:Rectangle = getBounds(this, sHelperRect); 
            if (bound != null &&
				bound.containsPoint(localPoint) )
			{
				return this;
			}
            else
        ...
        {
            return MatrixUtil.transformCoords(worldTransform, localPoint.x, localPoint.y, resultPoint);
        }
        ...
        {
			sHelperMatrix.copyFrom(worldTransform);
            sHelperMatrix.invert();
		...
		public function updateX(dt:Number):void
		{
			
		}
		
		public function renderUnify(support:RenderSupport):void
		{
            var isSoftwareMode : Boolean = Starling.current.isSoftWareMode;
            if ( mFilterEnable && mFilter.enable && !isSoftwareMode )
                mFilter.render ( support, parent.finalAlpha );
            else
                render(support, parent.finalAlpha);
		}
		
		public function updateUnify(dt:Number):void
		{			
			updateX(dt);	
		}
        ...
        [Inline] public function get hasVisibleArea():Boolean
        {
        ...
        public function get localTransform():Matrix
        {
            if (localMatrixDirty)
            {
                ...
                if (mSkewX == 0.0 && mSkewY == 0.0)
                {
                    ...
                    if (mRotation == 0.0)
                    {
                        mLocalMatrix.setTo(mScaleX, 0.0, 0.0, mScaleY,
                    ...
                    else
                    {
                        ...
                        mLocalMatrix.setTo(a, b, c, d, tx, ty);
                    }
                }
                else
                {
                    mLocalMatrix.identity();
                    mLocalMatrix.scale(mScaleX, mScaleY);
                    MatrixUtil.skew(mLocalMatrix, mSkewX, mSkewY);
                    mLocalMatrix.rotate(mRotation);
                    mLocalMatrix.translate(mX, mY);
                    
                    if (mPivotX != 0.0 || mPivotY != 0.0)
                    {
                        // prepend pivot transformation
                        mLocalMatrix.tx = mX - mLocalMatrix.a * mPivotX
                                                      - mLocalMatrix.c * mPivotY;
                        mLocalMatrix.ty = mY - mLocalMatrix.b * mPivotX
                                                      - mLocalMatrix.d * mPivotY;
                    }
                }
            }
        ...
        public function set localTransform(matrix:Matrix):void
        {
         ...
         *  @default false */
        [Inline] public function get useHandCursor():Boolean { return mUseHandCursor; }
        ...
        [Inline] public function get width():Number { return getBounds(mParent, sHelperRect).width; }
        ...
        [Inline] public function get height():Number { return getBounds(mParent, sHelperRect).height; }
        ...
        [Inline] public function get x():Number { return mX; }
        ...
        [Inline] public function get y():Number { return mY; }
        ...
        [Inline] public function get pivotX():Number { return mPivotX; }
        ...
        [Inline] public function get pivotY():Number { return mPivotY; }
        ...
        [Inline] public function get scaleX():Number { return mScaleX; }
        ...
        public function get worldScaleX () : Number
        {
            if ( mParent != null )
            {
                return mScaleX * mParent.worldScaleX;
            }
        ...
        public function get worldScaleY () : Number
        {
            if ( mParent != null )
            {
                return mScaleY * mParent.worldScaleY;
            }
        ...
        public function get worldRotation () : Number
        {
            if ( mParent != null )
            {
                return mRotation + mParent.worldRotation;
            }
        ...
        [Inline] public function get scaleY():Number { return mScaleY; }
        ...
        [Inline] public function get skewX():Number { return mSkewX; }
        ...
        [Inline] public function get skewY():Number { return mSkewY; }
         ...
         *  in radians.) */
        [Inline] public function get rotation():Number { return mRotation; }
        ...
        [Inline] public function get alpha():Number { return mAlpha; }
        public function set alpha(value:Number):void 
        { 
            mAlpha = value < 0.0 ? 0.0 : (value > 1.0 ? 1.0 : value);
			finalAlphaDirty = true;
        }
        
        /** The visibility of the object. An invisible object will be untouchable. */
        [Inline] public function get visible():Boolean { return mVisible; }
        public function set visible(value:Boolean):void { mVisible = value; }
        
        /** Indicates if this object (and its children) will receive touch events. */
        [Inline] public function get touchable():Boolean { return mTouchable; }
        public function set touchable(value:Boolean):void { mTouchable = value; }
        
        /** The blend mode determines how the object is blended with the objects underneath. 
         *   @default auto
         *   @see QFLib.Graphics.RenderCore.starling.display.BlendMode */
        [Inline] public function get blendMode():String { return mBlendMode; }
        public function set blendMode(value:String):void { mBlendMode = value; }
        
        /** The name of the display object (default: null). Used by 'getChildByName()' of 
         *  display object containers. */
        [Inline] public function get name():String { return mName; }
        ...
        public function set filterEnable ( enable : Boolean ) : void
        {
            mFilterEnable = enable;
            if ( enable && mFilter == null )
            {
                mFilter = new ObjectFilter ( this );
            }
        }
        [Inline] public function get filterEnable () : Boolean { return mFilterEnable; }
        public function get objectFilter():ObjectFilter { return mFilter; }
        
        /** The display object container that contains this display object. */
        [Inline] public function get parent():DisplayObjectContainer { return mParent; }
        
        /** The topmost object in the display tree the object is part of. */
        [Inline] public function get base():DisplayObject
        {
		...
		protected function get localMatrixDirty():Boolean	{ return mLocalMatrixDirty; }
			
		protected function set localMatrixDirty(value:Boolean):void
		{
			mLocalMatrixDirty = value;
		...
		///实现ISceneNode接口
		public function isDrawSelf():Boolean
		{ 
			return true; 
		}
		...
		public function getChild(index:int):ISceneNode
		{
			return null;
		}
		
		public function get finalAlpha():Number
		{
			if(finalAlphaDirty)
			{
				updateFinalAlpha();
		...
		{
			if(worldMatrixDirty || localMatrixDirty)
			{
        ...
        [Inline]
        public function get isInRender():Boolean
		{
			return mIsInRender;
		}
        [Inline]
        public function set isInRender(inRender:Boolean):void
		{
			mIsInRender = inRender;
		}
		
		public function set renderQueueID(groupID:int):void
		{
			if (groupID < 0 || groupID >= RenderQueueGroup.MAX)
			{
				throw new RangeError("groupID range error! Must be 0 <= groupID < RenderQueueGroup.MAX!");
			}
			
			mRenderQueueID = groupID;
			
			for(var i:int = 0; i < childCount; ++i)
			{
				var child:ISceneNode = getChild(i);
				
				if (child.inheritRenderQueue)
				{
					child.renderQueueID = groupID;
				}
			}
		}
        [Inline]
        public function get renderQueueID():int
		{
			return mRenderQueueID;
		}

        [Inline]
        public function set inheritRenderQueue(inherit:Boolean):void
		{
			mInheritRenderQueue = inherit;
		}
        [Inline]
        public function get inheritRenderQueue():Boolean
		{
			return mInheritRenderQueue;
		}
		
		public function get isVisbile():Boolean
		{
			return hasVisibleArea;
		}

        [Inline]
        public function get finalAlphaDirty():Boolean
		{
			return mFinalAlphaDirty;
		}
        [Inline]
        public function set finalAlphaDirty(value:Boolean):void
		{
			mFinalAlphaDirty = value;
		}

        [Inline]
        public function get worldMatrixDirty():Boolean
		{
			return mWorldMatrixDirty;
		}
        [Inline]
        public function set worldMatrixDirty(value:Boolean):void
		{
			mWorldMatrixDirty = value;
		}
		
		protected function updateWorldTransform():void
		{
			if(parent == null)
				mWorldMatrix.copyFrom(localTransform);
			else
			{
				mWorldMatrix.copyFrom(parent.worldTransform);
				MatrixUtil.prependMatrix(mWorldMatrix, localTransform);
			}
			
			mWorldMatrixDirty = false;
		}
		
		protected function updateFinalAlpha():void
		{
			if(parent == null)
				mFinalAlpha = mAlpha;
			else
				mFinalAlpha = mAlpha * parent.finalAlpha;

			finalAlphaDirty = false;
		}

        [Inline]
        public function set preRender(func:Function):void
        {
            mPreRenderFunc = func;
        }

		public function addToRenderQueue(culler:ICuller, groups:RenderQueueGroup):void
        {
            ...
            if (isDrawSelf())
			{
                if (culler.isVisibleNode(this))
                {
                    // mark sceneNode as visible
                    isInRender = true;
                    if(mPreRenderFunc != null)  mPreRenderFunc();
                    groups.addNode(renderQueueID, this);
                }
                else
                {
                    // mark sceneNode as invisible
                    isInRender = false;
                }
            }

			for (var i:int = 0, cc:int = childCount; i < cc; ++i)
			{
				var child:ISceneNode = getChild(i);
				if (child.isVisbile)
				{
        ...
        [Inline]
		public function get usingCamera():ICamera
		{
			return mUsingCamera;
		}
        [Inline]
        public function set usingCamera(c:ICamera):void
		{
			mUsingCamera = c;
		}
        ...
        public function set layer(value:uint):void
        {
            if (value > 32)
            {
                throw ArgumentError("Layer should not greater than 32!");
            }
            mLayer = value;
        }

        public function getLocalBound(resultRect:Rectangle=null):Rectangle
		{
			return getBounds(this, resultRect);
		}
		
		public function getWorldBound(resultRect:Rectangle=null):Rectangle
		{
			resultRect = getBounds( this, resultRect );
			MatrixUtil.transformRectangle(this.worldTransform, resultRect);
			return resultRect;
		}
    }
}