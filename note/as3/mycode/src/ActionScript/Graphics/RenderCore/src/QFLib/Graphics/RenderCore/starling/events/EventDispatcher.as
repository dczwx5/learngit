...
package QFLib.Graphics.RenderCore.starling.events
{
     ...
     *  
     *  @see Event
     *  @see QFLib.Graphics.RenderCore.starling.display.DisplayObject DisplayObject
     */
    public class EventDispatcher
    {
		private var mEventPoolDic:Dictionary = new Dictionary();
        private var mEventListeners:Dictionary;

		private var mEventCounts:Object = new Object();
		...
		//-- add some methods of optimization
		
		[inline]
		private function getListners(type:String):Vector.<Function>
		{
			return mEventListeners[type] as Vector.<Function>;
		}
		
		public function hasListener(type:String, func:Function):Boolean
		{
			var vec:Vector.<Function> = getListners(type);
			return vec ? vec.indexOf(func) != -1 : false;
		}
		
		[Inline]
		final public function getEvent(eventType:Class, eventName:String):Event
		{
			var pooledEvent:Event = mEventPoolDic[eventType];
			if(pooledEvent==null) mEventPoolDic[eventType] = pooledEvent = new eventType(eventName, null);
			else pooledEvent.reset(eventName, false, null);
			return pooledEvent;
		}
		
		/**
		 *Dispatches an event with the given class and event's type, event's data
		 * 
		 * @param eventType 事件的class定义
		 * @param eventName 事件名
		 * @param data 事件附带数据
		 * 
		 */		
		public function dispatchEventPooled(eventType:Class, eventName:String, data:*=null):void
		{
			var pooledEvent:Event = getEvent(eventType, eventName);
			pooledEvent.data = data;
			dispatchEvent(pooledEvent);
		}
		
		public function removeAllEvents():void
		{
			var had:Boolean = true;
			while (had)
			{
				had = false;
				for (var type:String in mEventListeners) 
				{
					var listeners:Vector.<Function> = mEventListeners[type] as Vector.<Function>;
					listeners.length = 0;
					delete mEventListeners[type];
		...
		public function showAllEventCount():void
		{
			for (var type:String in mEventListeners)
			{
				var listeners:Vector.<Function> = mEventListeners[type] as Vector.<Function>;
				//trace("事件类型：" + type + " 事件数量：" + listeners.length.toString());
				var preCount:int = 0;
				if (type in mEventCounts)
				{
					preCount = mEventCounts[type];
					if (listeners.length - preCount > 0)
					{
						trace(trace("事件类型：" + type + " 总体数量：" + listeners.length.toString() + " 事件递增：" + (listeners.length - preCount).toString()));
					}
				}

				mEventCounts[type] = listeners.length;
			}
		}
    }
}