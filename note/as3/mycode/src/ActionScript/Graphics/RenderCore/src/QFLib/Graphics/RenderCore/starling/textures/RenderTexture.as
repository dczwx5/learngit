...
import flash.display3D.VertexBuffer3D;
    ...
    public class RenderTexture extends SubTexture
    {
		...
		private var _legalWidth:int;
		private var _legalHeight:int;
        ...
        public function RenderTexture(width:int, height:int, persistent:Boolean=true, scale:Number=-1)
        {
            ...
            var legalWidth:int  = _originalWidth = width;
            var legalHeight:int = _originalHeight = height;
			...
			_legalWidth = legalWidth;
			_legalHeight = legalWidth;

            mActiveTexture = RenderTexturePool.instance().empty(legalWidth, legalHeight, PMA, false, true, scale);
            ...
            if (persistent)
            {
                mBufferTexture = RenderTexturePool.instance().empty(legalWidth, legalHeight, PMA, false, true, scale);
        ...
        {
            mSupport.dispose();
            RenderTexturePool.instance().recycleTexture(mActiveTexture);

            if (isPersistent) 
            {
                RenderTexturePool.instance().recycleTexture(mBufferTexture);
...
//            mSupport.blendMode = object.blendMode;
			mSupport.blendMode = object.blendMode == BlendMode.AUTO ? BlendMode.NORMAL : object.blendMode;
        ...
        private function renderBundled(renderBlock:Function, object:DisplayObject=null,
                                       ...
                                       antiAliasing:int=0):void
        {
            if (!Starling.current.contextValid) throw new MissingContextError();
            ...
            mSupport.pushRenderTarget(mActiveTexture, antiAliasing);
            mSupport.clear();
        ...
        public function clear(rgb:uint=0, alpha:Number=0.0):void
        {
            ...
            mSupport.pushRenderTarget(mActiveTexture);
        ...
        private function get supportsNonPotDimensions():Boolean
        {
            ...
            if (support == null)
            {
                if (target.isProfileBaselineConstrained() && target.isCreateRectangleTextureInContext())
                {
                    ...
                    try
                    {
                        texture = target.createRectangleTexture(2, 3, "compressedAlpha", true);
                        var preRenderTarget:Texture = Starling.current.support.renderTarget;
                        target.setRenderToTexture(texture);
                        target.clear();
                        if (preRenderTarget)
                        {
                            target.setRenderToTexture(preRenderTarget.base);
                        }
                        else
                        {
                            target.setRenderToBackBuffer();
                        }
                        target.createVertexBuffer(1, 1);
                        support = true;
                    }
                    catch (e:Error)
                    {
		...
		/** The value of the before legal width. */
		public function get originalWidth():int { return _originalWidth; }
		
		/** The value of the before legal height. */
		public function get originalHeight():int { return _originalHeight; }
		
		/** The value of the after legal original width. */
		public function get legalWidth():int { return _legalWidth; }
		
		/** The value of the after legal original height. */
		public function get legalHeight():int { return _legalHeight; }
    }
}