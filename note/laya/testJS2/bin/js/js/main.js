...
function initScreen(){
	...
	wallCtx = new CContext(); // $("#wallCanvas")[0].getContext("2d");
	grassCtx = new CContext(); // $("#grassCanvas")[0].getContext("2d");
	wallCtx.width = SCREEN_WIDTH;
	wallCtx.height = SCREEN_HEIGHT;
	grassCtx.width = SCREEN_WIDTH;
	grassCtx.height = SCREEN_HEIGHT;
// $("#wallCanvas").attr({"width":SCREEN_WIDTH});
	...
	tankCtx = new CContext(); // $("#tankCanvas")[0].getContext("2d");
	tankCtx.width = SCREEN_WIDTH;
	tankCtx.height = SCREEN_HEIGHT;
	// $("#tankCanvas").attr({"width":SCREEN_WIDTH});
	// $("#tankCanvas").attr({"height":SCREEN_HEIGHT});
	overCtx = new CContext(); // $("#overCanvas")[0].getContext("2d");
	overCtx.width = SCREEN_WIDTH;
	overCtx.height = SCREEN_HEIGHT;

	var root = getStage();
	root.addChild(ctx);
	ctx.addChild(wallCtx);
	ctx.addChild(grassCtx);
	ctx.addChild(tankCtx);
	ctx.addChild(overCtx);
...
function initObject(){
	...
	playControlView = new CPlayControlView();
	ctx.addChild(playControlView.getDisplayObject());
...
function processMove() {
	try {
		if (playControlView.isMove()) {
			player1.dir = playControlView.moveDir();
			player1.hit = false;
			player1.move();
		} else {
