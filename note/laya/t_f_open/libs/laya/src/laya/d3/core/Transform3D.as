package laya.d3.core {
	...
	public class Transform3D extends EventDispatcher {
		...
		/**@private */
		public static const TRANSFORM_WORLDROTATION:int = 0x10;
		/**@private */
		...
		public function get rotation():Quaternion {
			if (_getTransformFlag(TRANSFORM_WORLDROTATION)) {
				if (_parent != null)
				...
				_setTransformFlag(TRANSFORM_WORLDROTATION, false);
			}
			return _rotation;
		}
		...
		public function set rotation(value:Quaternion):void {
			...
			_rotation.getYawPitchRoll(_rotationEuler);
		...
		public function Transform3D(owner:Sprite3D) {
			...
			_setTransformFlag(TRANSFORM_WORLDPOSITION | TRANSFORM_WORLDROTATION | TRANSFORM_WORLDSCALE | TRANSFORM_WORLDMATRIX, true);
		}
		...
		private function _onWorldPositionRotationTransform():void {
			if (!_getTransformFlag(TRANSFORM_WORLDMATRIX) || !_getTransformFlag(TRANSFORM_WORLDPOSITION) || !_getTransformFlag(TRANSFORM_WORLDROTATION)) {
				_setTransformFlag(TRANSFORM_WORLDMATRIX | TRANSFORM_WORLDPOSITION | TRANSFORM_WORLDROTATION, true);
		...
		private function _onWorldRotationTransform():void {
			if (!_getTransformFlag(TRANSFORM_WORLDMATRIX) || !_getTransformFlag(TRANSFORM_WORLDROTATION)) {
				_setTransformFlag(TRANSFORM_WORLDMATRIX | TRANSFORM_WORLDROTATION, true);
				event(Event.TRANSFORM_CHANGED,_transformFlag);
				for (var i:int = 0, n:int = _children.length; i < n; i++)
		...
		public function _onWorldTransform():void {
			if (!_getTransformFlag(TRANSFORM_WORLDMATRIX) || !_getTransformFlag(TRANSFORM_WORLDPOSITION) || !_getTransformFlag(TRANSFORM_WORLDROTATION) || !_getTransformFlag(TRANSFORM_WORLDSCALE)) {
				_setTransformFlag(TRANSFORM_WORLDMATRIX | TRANSFORM_WORLDPOSITION | TRANSFORM_WORLDROTATION | TRANSFORM_WORLDSCALE, true);
				event(Event.TRANSFORM_CHANGED,_transformFlag);
				for (var i:int = 0, n:int = _children.length; i < n; i++)
