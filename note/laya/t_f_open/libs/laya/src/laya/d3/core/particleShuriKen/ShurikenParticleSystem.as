			...
			if (value) {
				...
				case 1: 
					if (rotationSeparate) {
						...
						shaDat.setBuffer(ShuriKenParticle3D.ROLANGULARVELOCITYGRADIENTW, rotation.gradientW._elements);
					} else {
				...
				if (renderMode === 4) {
					if(mesh){
						...
						if (vertexBufferCount > 1) {
							...
							_vertexBuffer = new VertexBuffer3D(vertexDeclaration.vertexStride*lastVBVertexCount, WebGLContext.DYNAMIC_DRAW);
					...
					_timeIndex = 11;
					_vertexStride = 4;						
					_vertexBuffer = new VertexBuffer3D(vertexDeclaration.vertexStride*_bufferMaxParticles * _vertexStride, WebGLContext.DYNAMIC_DRAW);
					_vertexBuffer.vertexDeclaration = vertexDeclaration;
