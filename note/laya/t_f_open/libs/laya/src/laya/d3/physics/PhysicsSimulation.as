package laya.d3.physics {
	...
	public class PhysicsSimulation {
		...
		/**@private	*/
		public var _dynamicPhysicsColliders:Vector.<PhysicsCollider> = new Vector.<PhysicsCollider>();
		/**@private	*/
		 ...
		 * @private
		 */
		public function _updateDynamicPhysicsColliders():void {//TODO:必须调整为移动才更新,否则太费性能
			for (var i:int = 0, n:int = _dynamicPhysicsColliders.length; i < n; i++) {
				var physicCollider:PhysicsComponent = _dynamicPhysicsColliders[i];
				physicCollider._derivePhysicsTransformation(physicCollider._nativeColliderObject.getWorldTransform(), false);
			}
		}
		...
		public function _eventScripts():void {
			...
			for (var i:int = 0, n:int = _currentFrameCollisions.length; i < n; i++) {
				...
				if (loopCount - curFrameCol._lastUpdateFrame === 1) {
					var ownerA:Sprite3D = curFrameCol._colliderA.owner as Sprite3D;
					...
					var ownerB:Sprite3D = curFrameCol._colliderB.owner as Sprite3D;
					...
					ownerA = curFrameCol._colliderA.owner as Sprite3D;
					...
					ownerB = curFrameCol._colliderB.owner as Sprite3D;
			...
			for (i = 0, n = _previousFrameCollisions.length; i < n; i++) {
				...
				if (preFrameCol._colliderA.destroyed || preFrameCol._colliderB.destroyed)//TODO:需要严查机制
					continue;
				if (loopCount - preFrameCol._updateFrame === 1) {
					_collisionsUtils.recoverCollision(preFrameCol);//回收collision对象
					ownerA = preFrameCol._colliderA.owner as Sprite3D;
					scriptsA = ownerA._scripts;
					if (scriptsA) {
					...
					ownerB = preFrameCol._colliderB.owner as Sprite3D;
					scriptsB = ownerB._scripts;
					if (scriptsB) {
						if (preFrameCol._isTrigger) {
							if (ownerB._needProcessTriggers) {
