package laya.d3.core {
	...
	public class Camera extends BaseCamera {
		...
		private function _calculationViewport(normalizedViewport:Viewport, width:int, height:int):void {
			_viewport.x = Math.min(Math.max(normalizedViewport.x * width, 0), width);
			_viewport.y = Math.min(Math.max(normalizedViewport.y * height, 0), height);
		 ...
		 * @inheritDoc
		 */
		override public function render(shader:Shader3D = null):void {
			...
			scene._clear(gl, context);
			scene._renderScene(gl, context, shader);
