package {
	...
	public class Laya3D {
		...
		/**@private */
		public static var isAbsolute:Boolean = false;
		/**@private */
		...
		private static function formatRelativePath(base:String, value:String):String {
			...
			var fullPath:String = Laya3D.isAbsolute ? value : base + value;
		 ...
		 *@private
		 */
		private static function _parseMaterial(mat:Object, secondLevelUrls:Array, urlMap:Array, urlVersion:String, hierarchyBasePath:String):void {
			mat.path = _addHierarchyInnerUrls(secondLevelUrls, urlMap, urlVersion, hierarchyBasePath, mat.path, Laya3D.MATERIAL);
		}
		...
		private static function _getSprite3DHierarchyInnerUrls(node:Object, firstLevelUrls:Array, secondLevelUrls:Array, thirdLevelUrls:Array, fourthLelUrls:Array, subUrls:Array, urlVersion:String, hierarchyBasePath:String):void {
			...
			case "Scene3D": //TODO:应该自动序列化类型
				...
				var skyboxMaterial:Object = props.skyboxMaterial;
				(skyboxMaterial) && (_parseMaterial(skyboxMaterial, secondLevelUrls, subUrls, urlVersion, hierarchyBasePath));
				break;
			case "Camera": 
				var skyboxMatData:Object = props.skyboxMaterial;
				(skyboxMatData) && (_parseMaterial(skyboxMatData, secondLevelUrls, subUrls, urlVersion, hierarchyBasePath));
				break;
			case "TrailSprite3D": 
			case "MeshSprite3D": 
			case "SkinnedMeshSprite3D": 
				...
				if (materials)
					for (i = 0, n = materials.length; i < n; i++)
						_parseMaterial(materials[i], secondLevelUrls, subUrls, urlVersion, hierarchyBasePath);
				break;
			case "ShuriKenParticle3D": 
				...
				_parseMaterial(props.material, secondLevelUrls, subUrls, urlVersion, hierarchyBasePath);
				break;
			case "Terrain": 
		...
		private static function _onHierarchyInnerFirstLevResouLoaded(loader:Loader, processHandler:Handler, lhData:Object, subUrls:Array):void {
			...
			var item:Node = Sprite3D._parse(lhData, loader._propertyParams, loader._constructParams);
			//(cache) && (cacheRes(formarUrl, item));
			//item._setUrl(loader.url);
		...
		private static function _onMeshLmLoaded(loader:Loader, lmData:ArrayBuffer):void {
			...
			var readData:Byte = new Byte(lmData);
			readData.pos = 0;
			...
			case "LAYAMODEL:0301": 
			case "LAYAMODEL:0400": 
				var dataOffset:uint = readData.getUint32();
				readData.pos = readData.pos + 4;//跳过数据信息区
				
				count = readData.getUint16();//跳过内容段落信息区
				readData.pos = readData.pos + count * 8;
				
				var offset:uint = readData.getUint32();//读取字符区
				count = readData.getUint16();
				readData.pos = dataOffset + offset;
				
				urls = [];
				for (i = 0; i < count; i++) {
					var string:String = readData.readUTFString();
		...
		private static function _loadTexture2D(loader:Loader):void {
			...
			var ext:String = url.substr(index, url.length - index);
			var type:String;
			switch (ext) {
			case "jpg": 
			case "jpeg": 
