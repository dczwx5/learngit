package laya.d3.physics {
	...
	public class PhysicsComponent extends Component {
		...
		public function get isActive():Boolean {
			return _nativeColliderObject ? _nativeColliderObject.getIsActive() : false;
		}
		...
		public function set canCollideWith(value:int):void {
			if (_canCollideWith !== value) {
				...
				var broadphase:* = _nativeColliderObject.getBroadphaseHandle();
		 ...
		 *	通过渲染矩阵更新物理矩阵。
		 */
		public function _derivePhysicsTransformation(physicTransformOut:*, force:Boolean):void {
			if (force || _getTransformFlag(Transform3D.TRANSFORM_WORLDMATRIX)) {
				...
				if (force || _getTransformFlag(Transform3D.TRANSFORM_WORLDROTATION)) {
					...
					_setTransformFlag(Transform3D.TRANSFORM_WORLDROTATION, false);
				}
		...
		override protected function _onDestroy():void {
			...
			physics3D.destroy(_colliderShape);
			super._onDestroy();
		...
		public function _onTransformChanged(flag:int):void {
			_transformFlag |= flag;//TODO:rigidbody如何处理,一直在调用,此类为公用
		}
		...
		public function activate(forceActivation:Boolean = false):void {
			_nativeColliderObject && (_nativeColliderObject.activate(forceActivation));
		...
		override public function _cloneTo(dest:Component):void {
			...
			destPhysicsComponent.colliderShape = _colliderShape.clone();
		}
	}
}