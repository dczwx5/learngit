package laya.d3.graphics {
	...
	public class IndexBuffer3D extends Buffer {
		 ...
		 * @return	    索引缓冲。
		 */
		public static var create:Function = function(indexType:String, indexCount:int, bufferUsage:int =0x88E4/*WebGLContext.STATIC_DRAW*/, canRead:Boolean = false):IndexBuffer3D {
		...
		public function IndexBuffer3D(indexType:String, indexCount:int, bufferUsage:int =0x88E4/*WebGLContext.STATIC_DRAW*/, canRead:Boolean = false) {
			...
			if (canRead) {
				...
				_setGPUMemory(byteLength * 2);//可读内存CPU、GPU各占一份
			} else {
		...
		override public function bind():Boolean {
			WebGLContext.bindVertexArray(LayaGL.instance, null);//避免绑定到局部VAO中
