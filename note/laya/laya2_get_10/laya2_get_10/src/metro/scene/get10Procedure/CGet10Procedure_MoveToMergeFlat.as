...
public class CGet10Procedure_MoveToMergeFlat extends CProcedureBase {
	...
	private function _moveToMergerFlag() : void {
		...
		var subY:int;
		for (key in m_handler.selectMap) {
			child = m_handler.container.getChildAt(key) as CFlatObejct;
			if (m_selectItem != child) {
				subX = Math.abs(child.getIndexX() - m_selectItem.getIndexX());
				subY = Math.abs(child.getIndexY() - m_selectItem.getIndexY());
				if (subX + subY > maxDist) {
					maxDist = subX + subY;
				}	
			}
		}
		...
		for (key in m_handler.selectMap) {
			...
			if (m_selectItem != child) {
				...
				subX = Math.abs(child.getIndexX() - m_selectItem.getIndexX());
				subY = Math.abs(child.getIndexY() - m_selectItem.getIndexY());
				var hideTime:Number = CFlatObejct.FALL_TIME * (maxDist - (subX + subY) + 1)/maxDist;
				Tween.to(child, {alpha:0}, hideTime,
					 Ease.bounceInOut, Handler.create(this, _onFinished));
			}
		}
	}
