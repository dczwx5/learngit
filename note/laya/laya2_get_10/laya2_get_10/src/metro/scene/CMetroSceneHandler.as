package metro.scene
{
	...
	import metro.scene.flat.CFlatBuilder;
	import metro.scene.flat.CFlatLock;
	import metro.scene.get10Procedure.CGet10Procedure_CheckAndOpenNewLock;
	...
	public class CMetroSceneHandler extends CBean {
		...
		protected override function onAwake() : void {
			super.onAwake();
			addBean(m_flatBuilder = new CFlatBuilder());			
		}
		protected override function onStart() : Boolean {
			...
			m_pPlaySystem = system.stage.getSystem(CPlayerSystem) as CPlayerSystem;
		...
		public function createScene(container:CBaseDisplay, effectLayer:CBaseDisplay) : void {
			...
			var procedureList:Array = [
				...
				new CGet10Procedure_SelectLock(), new CGet10Procedure_CheckAndOpenNewLock()
			];
		...
		public function open(openFlat:CFlatObejct) : void {
			var cost:int = m_pPlaySystem.playerData.getOpenCost();
			var pPlayerSystem:CPlayerSystem = m_pPlaySystem;
			if (pPlayerSystem.netHandler.addScore(-cost)) {
				pPlayerSystem.playerData.openNewLock();
				if (openFlat.isLockState) {
					(openFlat.fsm.currentState as CFlatLock).changeToReady();
				}
		...
		public function get flatBuilder() : CFlatBuilder {
			return m_flatBuilder;
		}
		...
		private var m_pPlaySystem:CPlayerSystem;

		private var m_flatBuilder:CFlatBuilder;
	}
