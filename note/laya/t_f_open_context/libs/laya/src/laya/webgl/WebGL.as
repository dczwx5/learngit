	...
	public class WebGL {
		...
		public static function _nativeRender_enable():void 
		{
			...
			HTMLImage.create = function(width:int,height:int):Bitmap {
				return new Texture2D(width,height,BaseTexture.FORMAT_R8G8B8A8, false, false);
			}
			...
			Filter._filter = function(sprite:Sprite, context:*, x:Number, y:Number):void {
				...
				if (next) {
					...
					if (!out || sprite.getRepaint() != 0) {
						...
						var source:RenderTexture2D = WebGLRTMgr.getRT(b.width, b.height);
						var outRT:RenderTexture2D = WebGLRTMgr.getRT(b.width, b.height);
