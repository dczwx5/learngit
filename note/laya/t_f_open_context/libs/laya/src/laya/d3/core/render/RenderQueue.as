package laya.d3.core.render {
	...
	public class RenderQueue {
		...
		private function _compare(left:RenderElement, right:RenderElement):int {
			...
			if (renderQueue === 0) {
				return isTransparent ? right.render._distanceForSort - left.render._distanceForSort : left.render._distanceForSort - right.render._distanceForSort;
			} else {
				return renderQueue;
			}
		}
			...
			for (var i:int = 0, n:int = elements.length; i < n; i++) {
				...
				if (geometry._prepareRender(context)) {
					...
					for (var j:int = 0, m:int = passes.length; j < m; j++) {
						...
						geometry._bufferState.bindBufferState();
