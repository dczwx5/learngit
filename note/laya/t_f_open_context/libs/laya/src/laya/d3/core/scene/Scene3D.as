package laya.d3.core.scene {
	...
	public class Scene3D extends Sprite implements ISubmit, ICreateResource {
		...
		public static const SPOTLIGHTSPOTANGLE:int = 11;
		...
		public static const REFLECTIONTEXTURE:int = 22;
		public static const REFLETIONINTENSITY:int = 23;
		public static const TIME:int = 24;
		public static const ANGLEATTENUATIONTEXTURE:int = 25;
		public static const RANGEATTENUATIONTEXTURE:int = 26;
		public static const POINTLIGHTMATRIX:int = 27;
		public static const SPOTLIGHTMATRIX:int = 28;
		...
		/**当前创建精灵所属遮罩层。*/
		public var currentCreationLayer:Layer;
		/** 是否启用灯光。*/
		...
		public function Scene3D() {
			...
			_shaderValues = new ShaderData(null, 22);
			...
			var rangeAttenTex:Texture2D = Utils3D._buildTexture2D(1024, 1, BaseTexture.FORMAT_ALPHA8, TextureGenerator.lightAttenTexture);//TODO:移动位置
			rangeAttenTex.wrapModeU = BaseTexture.WARPMODE_CLAMP;
			rangeAttenTex.wrapModeV = BaseTexture.WARPMODE_CLAMP;
			_shaderValues.setTexture(Scene3D.RANGEATTENUATIONTEXTURE, rangeAttenTex);
		...
		private function _update():void {
			...
			if (Laya3D._enbalePhysics && !PhysicsSimulation.disableSimulation) {
				simulation._updateDynamicPhysicsColliders();
				//simulate physics
		 ...
		 * @private
		 */
		public function _initLayer():void {
			_layerList.length = Layer.maxCount;
			for (var i:int = 0; i < Layer.maxCount; i++) {
		...
		private function _updateScript():void {
			...
			for (var i:int = 0, n:int = pool.length; i < n; i++) {
				var script:Script = elements[i] as Script;
		...
		public function _clear(gl:WebGLContext, state:RenderContext3D):void {
			...
			var vpY:Number = camera._canvasHeight - viewport.y - vpHeight;
			gl.viewport(vpX, vpY, vpWidth, vpHeight);
			var flag:int = WebGLContext.DEPTH_BUFFER_BIT;
			
			var clearFlag:int = camera.clearFlag;
			if (clearFlag === BaseCamera.CLEARFLAG_SKY && !(camera.skyboxMaterial || _skyboxMaterial))
		 ...
		 * @private
		 */
		public function _renderScene(gl:WebGLContext, state:RenderContext3D, customShader:Shader3D = null):void {
			var camera:BaseCamera = state.camera;
			var position:Vector3 = camera.transform.position;
			camera.renderTarget ? _opaqueQueue._render(state, true, customShader) : _opaqueQueue._render(state, false, customShader);//非透明队列
			if (camera.clearFlag === BaseCamera.CLEARFLAG_SKY) {
				var skyMaterial:BaseMaterial = camera.skyboxMaterial || _skyboxMaterial;
				if (skyMaterial) {
					WebGLContext.setCullFace(gl, false);
					WebGLContext.setDepthFunc(gl, WebGLContext.LEQUAL);
					WebGLContext.setDepthMask(gl, false);
					var shader:ShaderInstance = state.shader = skyMaterial._shader._passes[0].withCompile(0, 0, 0);
					var forceUploadParams:Boolean = shader.bind() || (Stat.loopCount !== shader._uploadLoopCount);
					
					var skyBox:SkyBox = SkyBox._instance;
					...
					skyBox._bufferState.bindBufferState();
					skyBox._render(state);
					WebGLContext.setDepthFunc(gl, WebGLContext.LESS);
					WebGLContext.setDepthMask(gl, true);
				}
			}
			camera.renderTarget ? _transparentQueue._render(state, true, customShader) : _transparentQueue._render(state, false, customShader);//透明队列
		}
		...
		override public function _parse(data:Object):void {
			...
			var skyboxMaterialData:Object = data.skyboxMaterial;
			(skyboxMaterialData) && (skyboxMaterial = Loader.getRes(skyboxMaterialData.path));
		...
		public function isLayerVisible(layer:int, camera:Camera):Boolean {
			var mask:int = _layerList[layer]._mask;
			return (mask & camera.cullingMask) != 0;
		}
		
		/**
		 * @private
		 */
		public function _getLayer(layer:int):Layer {
			return _layerList[layer];
		}
