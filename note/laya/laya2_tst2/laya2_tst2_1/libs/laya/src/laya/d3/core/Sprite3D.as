package laya.d3.core {
	...
	import laya.d3.shader.ShaderUniforms;
	...
	public class Sprite3D extends Node implements ICreateResource, IClone {
		...
		/**@private 着色器变量名，世界矩阵。*/
		public static var WORLDMATRIX:int;
		/**@private 着色器变量名，世界视图投影矩阵。*/
		public static var MVPMATRIX:int;
		
		/**@private */
		protected static var _uniqueIDCounter:int = 0;
		
		/**@private */
		public static var shaderUniforms:ShaderUniforms = new ShaderUniforms();
		
		/**
		 * @private
		 */
		public static function __init__():void {
			WORLDMATRIX = shaderUniforms.registerUniform();
			MVPMATRIX = shaderUniforms.registerUniform();
		}
		...
		override public function _parse(data:Object):void {
			_isStatic = data.isStatic;
			var activeData:* = data.active;
			(activeData!==undefined)&&(active =activeData);
			name = data.name;
			var loccalPosition:Vector3 = transform.localPosition;
			loccalPosition.fromArray(data.position || data.translate);//data.translate兼容性
			...
			(layerData != null) && (layer = layerData);
		}
