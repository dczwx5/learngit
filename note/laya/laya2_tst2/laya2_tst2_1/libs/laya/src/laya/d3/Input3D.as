package laya.d3 {
	...
	public class Input3D {
		 ...
		 * @private
		 */
		private function _mouseTouchRayCast():void {
			...
			var cameraPool:Vector.<BaseCamera> = _scene._cameraPool;
			touchPosE[0] = x;
			touchPosE[1] = y;
			for (var i:int = cameraPool.length - 1; i >= 0; i--) {
				var camera:Camera = cameraPool[i] as Camera;
		 ...
		 */
		public function _changeTouches(changedTouches:Array, pixelRatio:int, flag:int):void {
			...
			for (var j:int = 0, m:int = changedTouches.length; j < m; j++) {
				...
				var posX:Number = nativeTouch.pageX * pixelRatio;
				var posY:Number = nativeTouch.pageY * pixelRatio;
				switch (flag) {
				case 0://add 
					_touches.add(touch);
					offsetX += posX;
		...
		public function _update():void {
			...
			var pixelRatio:int = Browser.pixelRatio;//考虑DPI对实际物理像素影响
			n = _eventList.length;
			if (n > 0) {
				for (i = 0; i < n; i++) {
					var e:* = _eventList[i];
					...
					case "mouseup": 
						...
						break;
					case "mousemove": 
						_mouseTouch.mousePositionX = e.pageX * pixelRatio;
						_mouseTouch.mousePositionY = e.pageY * pixelRatio;
						_mouseTouchRayCast();
						break;
					case "touchstart": 
						var lastLength:int = _touches.length;
						_changeTouches(e.changedTouches, pixelRatio, 0);
						_mouseTouchRayCast();//触摸点击时touchMove不会触发,需要调用_touchRayCast()函数
						(lastLength === 0) && (_mouseTouchDown());
						break;
					case "touchend": 
					case "touchcancel": 
						_changeTouches(e.changedTouches, pixelRatio, 1);
						(_touches.length === 0) && (_mouseTouchUp());
						break;
					case "touchmove": 
						_changeTouches(e.changedTouches, pixelRatio, 2);
						_mouseTouchRayCast();
						break;
					default: 
