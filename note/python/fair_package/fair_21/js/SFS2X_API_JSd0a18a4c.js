...
SFS2X.SmartFox.prototype.connect = function(a, b, c) {
	...
	else if(this._socketEngine.isConnecting) this._log.warn("A connection attempt is already in progress");
	else {
...
function rstr2any(a, b) {
	var c = b.length,
		d, e, f, g, i, j = Array(Math.ceil(a.length / 2));
...
function binl_md5(a, b) {
	a[b >> 5] |= 128 << b % 32;
	a[(b + 64 >>> 9 << 4) + 14] = b;
	for(var c = 1732584193, d = -271733879, e = -1732584194, f = 271733878, g = 0; g < a.length; g += 16) var i = c,
		j = d,
		k = e,
		h = f,
...
SFS2X.Utils.md5_crypt = function(a, b) {
	...
	for(var i = "$1$" + b + "$", g = str_md5(f), e = 0; 1E3 > e; e++) f = "", f = 0 != (e & 1) ? f + a : f + g, 0 != e % 3 && (f += b), 0 != e % 7 && (f +=
		a), f = 0 != (e & 1) ? f + g : f + a, g = str_md5(f);
	g += g.charAt(5);
...
SFS2X.Controllers.SystemController.prototype._initRequestHandlers = function() {
	...
	this._requestHandlers[SFS2X.Requests.ChangeRoomPassword] =
		"_fnChangeRoomPassword";
...
SFS2X.Controllers.SystemController.prototype._fnSetUserPosition = function(a) {
	...
	if(null != f)
		for(h = 0; h < f.length; h++) e = c.getMMOItem(f[h]), null != e && (c._removeItem(e), k.push(e));
	if(null != a)
		for(h = 0; h < a.length; h++) e = a[h], f = SFS2X.Entities.MMOItem.fromArray(e), j.push(f), c._addMMOItem(f), e = e[2], null != e && (f.aoiEntryPoint = SFS2X.Entities.Data.Vec3D.fromArray(e));
	b.addedItems =
		j;
	b.removedItems = k;
	b.removedUsers = i;
...
SFS2X.Controllers.SystemController.prototype._fnUserLost = function(a) {
	...
	if(null != a) {
		...
		this._sfs.userManager._removeUserReference(a, !0);
...
SFS2X.Controllers.SystemController.prototype._fnBlockBuddy = function(a) {
	...
	if(null == a[SFS2X.Controllers.KEY_ERROR_CODE]) {
		...
		if(null != a[SFS2X.Requests.BuddyList.BlockBuddyRequest.KEY_BUDDY]) d = SFS2X.Entities.SFSBuddy.fromArray(a[SFS2X.Requests.BuddyList.BlockBuddyRequest.KEY_BUDDY]), this._sfs.buddyManager._addBuddy(d);
		else if(null != d) d.blocked = a[SFS2X.Requests.BuddyList.BlockBuddyRequest.KEY_BUDDY_BLOCK_STATE];
		else {
			this._log.warn("Unexpected: buddy blocking/unblocking failed, can't find " + c + " in local buddy list");
			return
		}
		b.buddy = d;
		this._sfs._dispatchEvent(SFS2X.SFSBuddyEvent.BUDDY_BLOCK, b)
	} else c = a[SFS2X.Controllers.KEY_ERROR_CODE], a = SFS2X.ErrorCodes.getErrorMessage(c, a[SFS2X.Controllers.KEY_ERROR_PARAMS]), b.errorMessage = a, b.errorCode = c, this._sfs._dispatchEvent(SFS2X.SFSBuddyEvent.BUDDY_ERROR, b)
};
SFS2X.Controllers.SystemController.prototype._fnGoOnline = function(a) {
	...
	if(null == a[SFS2X.Controllers.KEY_ERROR_CODE]) {
		var c = a[SFS2X.Requests.BuddyList.GoOnlineRequest.KEY_BUDDY_NAME],
			...
			i = !0;
		if(e) this._sfs.buddyManager.getMyOnlineState() != g && (this._log.warn("Unexpected: myOnlineState is not in synch with the server; resynching to: " +
			g), this._sfs.buddyManager._setMyOnlineState(g));
...
SFS2X.Controllers.SystemController.prototype._getOrCreateUser = function(a, b, c) {
	...
	if(null == d) d = SFS2X.Entities.SFSUser.fromArray(a, c), d._setUserManager(this._sfs.userManager);
	else if(null != c) {
...
SFS2X.Controllers.ExtensionController.prototype.handleMessage = function(a) {
	...
	if(null != a[this.KEY_ROOM]) {
		var a = a[this.KEY_ROOM],
			c = this._sfs.roomManager.getRoomById(a);
		b.sourceRoom = a;
		b.room = c
	}
...
SFS2X.Entities.SFSRoom = Class.extend({
	...
	_merge: function(a) {
		this.variables = [];
		for(var b in a.variables) this.variables[b] =
			a.variables[b];
...
SFS2X.Entities.Variables.BaseVariable = Class.extend({
	init: function(a, b, c) {
		this.name = a;
		this.value = b;
		this._setType(c)
	...
	_setType: function(a) {
		null == a && (a = -1); - 1 < a ? this.type = this.getTypeName(a) : null == this.value ? this.type = this.getTypeName(SFS2X.Entities.Variables.VariableType.NULL) :
			(a = typeof this.value, "boolean" == a ? this.type = this.getTypeName(SFS2X.Entities.Variables.VariableType.BOOL) : "number" == a ? (a = this.value, this.type = a === +a && a === (a | 0) ? this.getTypeName(SFS2X.Entities.Variables.VariableType.INT) : this.getTypeName(SFS2X.Entities.Variables.VariableType.DOUBLE)) : "string" == a ? this.type = this.getTypeName(SFS2X.Entities.Variables.VariableType.STRING) : "object" == a && (this.type = this.value instanceof Array ? this.getTypeName(SFS2X.Entities.Variables.VariableType.ARRAY) : this.getTypeName(SFS2X.Entities.Variables.VariableType.OBJECT)))
	},
	getTypeName: function(a) {
		return SFS2X.Entities.Variables.VariableType.getTypeName(a)
	}
});
SFS2X.Entities.Variables.SFSUserVariable = SFS2X.Entities.Variables.BaseVariable.extend({
	init: function(a, b, c) {
		this._super(a, b, c);
		this.isPrivate = !1
	},
...
SFS2X.Entities.Variables.SFSUserVariable.fromArray = function(a) {
	var b = new SFS2X.Entities.Variables.SFSUserVariable(a[0], a[2], a[1]);
	3 < a.length && (b.isPrivate = a[3]);
	return b
};
SFS2X.Entities.Variables.SFSRoomVariable = SFS2X.Entities.Variables.BaseVariable.extend({
	init: function(a, b, c) {
		this._super(a, b, c);
...
SFS2X.Entities.Variables.SFSBuddyVariable = SFS2X.Entities.Variables.BaseVariable.extend({
	init: function(a, b, c) {
		this._super(a, b, c)
	},
	toString: function() {
...
SFS2X.Entities.Variables.MMOItemVariable = SFS2X.Entities.Variables.BaseVariable.extend({
	init: function(a, b, c) {
		this._super(a, b, c)
	},
	toString: function() {
...
SFS2X.Entities.Invitation.InvitationReply.EXPIRED = 255;
...
SFS2X.Managers.GlobalUserManager = SFS2X.Managers.UserManager.extend({
	...
	_removeUser: function(a) {
		this._removeUserReference(a, !1)
	},
	_removeUserReference: function(a, b) {
		if(null != this._roomRefCount)
			if(1 > this._roomRefCount[a]) SFS2X.Logger.warn("GlobalUserManager RefCount is already at zero. User: " + a);
			else {
				if(this._roomRefCount[a]--, 0 == this._roomRefCount[a] ||
					b) this.__removeUser(a), delete this._roomRefCount[a]
			}
		else SFS2X.Logger.warn("Can't remove User from GlobalUserManager. RefCount missing. User: " + a)
	},
...
SFS2X.Managers.RoomManager.prototype.containsRoomInGroup = function(a, b) {
	...
	for(f in c) {
		var g = c[f];
		if(g instanceof SFS2X.Entities.SFSRoom)
			if(e) {
				if(g.id == a) {
					d = !0;
					break
...
SFS2X.Requests.System.CreateRoomRequest = SFS2X.Requests._BaseRequest.extend({
	...
	validate: function() {
		...
		if(0 < a.length) throw new SFS2X.Exceptions.SFSValidationError("CreateRoomRequest Error",
			a);
	},
	execute: function() {
		...
		this._reqObj[this.constructor.KEY_ALLOW_JOIN_INVITATION_BY_OWNER] =
			this._settings.allowOwnerOnlyInvitation;
		...
		null != this._settings.permissions && (a = [], a.push(this._settings.permissions.allowNameChange), a.push(this._settings.permissions.allowPasswordStateChange), a.push(this._settings.permissions.allowPublicMessages),
			a.push(this._settings.permissions.allowResizing), this._reqObj[this.constructor.KEY_PERMISSIONS] = a);
		...
		null != this._settings.extension && (this._reqObj[this.constructor.KEY_EXTID] = this._settings.extension.id, this._reqObj[this.constructor.KEY_EXTCLASS] =
			this._settings.extension.className, null != this._settings.extension.propertiesFile && 0 < this._settings.extension.propertiesFile.length && (this._reqObj[this.constructor.KEY_EXTPROP] = this._settings.extension.propertiesFile));
		this._settings instanceof SFS2X.Requests.MMO.MMORoomSettings && (this._reqObj[this.constructor.KEY_MMO_DEFAULT_AOI] = this._settings.defaultAOI._toArray(), null != this._settings.mapLimits && (this._reqObj[this.constructor.KEY_MMO_MAP_LOW_LIMIT] = this._settings.mapLimits.lowerLimit._toArray(),
			this._reqObj[this.constructor.KEY_MMO_MAP_HIGH_LIMIT] = this._settings.mapLimits.higherLimit._toArray()), this._reqObj[this.constructor.KEY_MMO_USER_MAX_LIMBO_SECONDS] = this._settings.userMaxLimboSeconds, this._reqObj[this.constructor.KEY_MMO_PROXIMITY_UPDATE_MILLIS] = this._settings.proximityListUpdateMillis, this._reqObj[this.constructor.KEY_MMO_SEND_ENTRY_POINT] = this._settings.sendAOIEntryPoint);
		this._reqObj[this.constructor.KEY_AUTOJOIN] = this._autoJoin;
		null != this._roomToLeave && (this._reqObj[this.constructor.KEY_ROOM_TO_LEAVE] =
			this._roomToLeave.id)
	}
});
...
SFS2X.Requests.System.CreateRoomRequest.KEY_ALLOW_JOIN_INVITATION_BY_OWNER = "aji";
...
SFS2X.Requests.System.GenericMessageRequest = SFS2X.Requests._BaseRequest.extend({
	...
	execute: function(a) {
		this._reqObj[SFS2X.Requests.System.GenericMessageRequest.KEY_MESSAGE_TYPE] =
			this._type;
		switch(this._type) {
			case SFS2X.Requests.GenericMessageType.PUBLIC_MSG:
	...
	_validateSuperUserMessage: function(a, b) {
		...
		switch(this._sendMode) {
			case SFS2X.Requests.MessageRecipientMode.TO_USER:
				this._recipient instanceof SFS2X.Entities.SFSUser ||
					b.push("SFSUser object expected as recipient");
				break;
	...
	_executeSuperUserMessage: function() {
		...
		switch(this._sendMode) {
			case SFS2X.Requests.MessageRecipientMode.TO_USER:
				...
				break;
			case SFS2X.Requests.MessageRecipientMode.TO_ROOM:
				this._reqObj[SFS2X.Requests.System.GenericMessageRequest.KEY_RECIPIENT] =
					this._recipient.id;
				break;
			case SFS2X.Requests.MessageRecipientMode.TO_GROUP:
				this._reqObj[SFS2X.Requests.System.GenericMessageRequest.KEY_RECIPIENT] = this._recipient
	...
	_executeObjectMessage: function(a) {
		...
		if(null != this._recipient && this._recipient instanceof Array) {
			...
			if(b.length > this._room.capacity) throw new SFS2X.Exceptions.SFSError("The number of recipients is bigger than the target Room capacity: " +
				b.length);
			for(var c in b) {
				var d = b[c];
...
SFS2X.Requests.System.KickUserRequest = SFS2X.Requests._BaseRequest.extend({
	...
	validate: function() {
		var a = [];
		if(0 < a.length) throw new SFS2X.Exceptions.SFSValidationError("KickUserRequest Error", a);
	},
	execute: function() {
		this._reqObj[this.constructor.KEY_USER_ID] = this._userId;
		this._reqObj[this.constructor.KEY_DELAY] = this._delay;
		null != this._message && 0 < this._message.length && (this._reqObj[this.constructor.KEY_MESSAGE] =
			this._message)
	}
});
...
SFS2X.Requests.System.BanUserRequest = SFS2X.Requests._BaseRequest.extend({
	...
	validate: function() {
		var a = [];
		if(0 < a.length) throw new SFS2X.Exceptions.SFSValidationError("BanUserRequest Error", a);
	},
	execute: function() {
		this._reqObj[this.constructor.KEY_USER_ID] = this._userId;
		this._reqObj[this.constructor.KEY_DELAY] = this._delay;
...
SFS2X.Requests.BuddyList.BlockBuddyRequest.KEY_BUDDY = "bd";
...
SFS2X.Requests.Game.CreateSFSGameRequest = SFS2X.Requests._BaseRequest.extend({
	...
	validate: function(a) {
		...
		null != this._settings.invitedPlayers && this._settings.invitedPlayers.length > SFS2X.Requests.Game.InviteUsersRequest.MAX_INVITATIONS_FROM_CLIENT_SIDE &&
			b.push("Can't invite more than " + SFS2X.Requests.Game.InviteUsersRequest.MAX_INVITATIONS_FROM_CLIENT_SIDE + " players from client side");
	...
	execute: function(a) {
		...
		this._reqObj[this.constructor.KEY_INVITATION_EXPIRY] =
			this._settings.invitationExpiryTime;
...
SFS2X.Requests.Game.QuickJoinGameRequest = SFS2X.Requests._BaseRequest.extend({
	...
	execute: function() {
		if("string" == typeof this._whereToSearch[0]) this._reqObj[this.constructor.KEY_GROUP_LIST] = this._whereToSearch;
...
SFS2X.Requests.Game.JoinRoomInvitationRequest = SFS2X.Requests._BaseRequest.extend({
	init: function(a, b, c, d, e) {
		this._super(SFS2X.Requests.JoinRoomInvite);
		this._targetRoom = a;
		this._invitedUserNames = b;
		this._params = c;
		this._expirySeconds = d;
		this._asSpectator = e
	},
	validate: function() {
		var a = [];
		null == this._targetRoom && a.push("Missing target room");
		(null == this._invitedUserNames || 1 > this._invitedUserNames.length) && a.push("No invitees provided");
		if(0 < a.length) throw new SFS2X.Exceptions.SFSValidationError("JoinRoomInvitationRequest Error", a);
	},
	execute: function() {
		this._reqObj[this.constructor.KEY_ROOM_ID] = this._targetRoom.id;
		this._reqObj[this.constructor.KEY_INVITED_NAMES] = this._invitedUserNames;
		this._reqObj[this.constructor.KEY_EXPIRY_SECONDS] = this._expirySeconds;
		this._reqObj[this.constructor.KEY_AS_SPECT] = this._asSpectator;
		null != this._params && (this._reqObj[this.constructor.KEY_OPTIONAL_PARAMS] = this._params)
	}
});
SFS2X.Requests.Game.JoinRoomInvitationRequest.KEY_ROOM_ID = "r";
SFS2X.Requests.Game.JoinRoomInvitationRequest.KEY_EXPIRY_SECONDS = "es";
SFS2X.Requests.Game.JoinRoomInvitationRequest.KEY_INVITED_NAMES = "in";
SFS2X.Requests.Game.JoinRoomInvitationRequest.KEY_AS_SPECT = "as";
SFS2X.Requests.Game.JoinRoomInvitationRequest.KEY_OPTIONAL_PARAMS = "op";
...
SFS2X.Requests.JoinRoomInvite = 304;
...
SFS2X.Requests.RoomSettings = Class.extend({
	init: function(a) {
		...
		this.allowOwnerOnlyInvitation = !0
	}
});
...
SFS2X.SocketEngine.prototype.connect = function(a, b, c) {
    ...
    SFS2X.SocketEngine._socket = this._socket;
};
...
SFS2X.SocketEngine.prototype._onSocketConnect = function () {
	var self = (this instanceof WebSocket) ? this : SFS2X.SocketEngine._socket;
	self._scope.isConnected = !0;
	self._scope.isConnecting = !1;
	self._scope._log.debug("SOCKET CONNECTION OK");
	self._scope._eventDispatcher.dispatchEvent(SFS2X.SocketEvent.CONNECT, {
		success: !0,
		isReconnection: self._scope._attemptingReconnection
	})
};
SFS2X.SocketEngine.prototype._onSocketDisconnect = function () {
	var self = (this instanceof WebSocket) ? this : SFS2X.SocketEngine._socket;
	self._scope.isConnected = !1;
	if (self._scope.isConnecting) {
		self._scope.isConnecting = !1;
		var a = {
			success: !1,
			isReconnection: self._scope._attemptingReconnection
		};
		self._scope._eventDispatcher.dispatchEvent(SFS2X.SocketEvent.CONNECT, a)
	} else a = {
		reason: self._scope._disconnectionReason
	}, self._scope._eventDispatcher.dispatchEvent(SFS2X.SocketEvent.DISCONNECT, a), self._scope._disconnectionReason = SFS2X.Utils.ClientDisconnectionReason.UNKNOWN
};
SFS2X.SocketEngine.prototype._onSocketData = function (a) {
	var self = (this instanceof WebSocket) ? this : SFS2X.SocketEngine._socket;
	self._scope._eventDispatcher.dispatchEvent(SFS2X.SocketEvent.DATA, self._scope._decodeMessage(a.data))
};
SFS2X.SocketEngine.prototype._onSocketError = function (a) {
	var self = (this instanceof WebSocket) ? this : SFS2X.SocketEngine._socket;
	self._scope._eventDispatcher.dispatchEvent(SFS2X.SocketEvent.IOERROR, a.data)
};
SFS2X.SocketEvent = {
